...
class VigorResponsiveDotStim(ContinuousRandomDotKinematogram):
    ...
    tail vigor exceeds `vigor_threshold`. Uses a small hysteresis so the
    image doesn't flicker near threshold.
    """
    ...
    class VigorResponsiveDotStim(ContinuousRandomDotKinematogram):
        ...
        def _switch_blackout(self, new_state: bool):
            """Helper to flip state and log the event with a timestamp."""
...
class VisualStim_dots(Protocol):
    ...
    stytra_config = dict(
        ...
        camera=dict(camera=dict(type="basler", cam_idx=0)),  # or device_idx=0 if your adapter uses that
    )

    def __init__(self):
        super().__init__()
        self.number_of_repeats = Param(1)
        self.left_right = [0, 3]  # 0 = right, 3 = left (Stytra’s convention)

        # UI parameters you might want to tweak
        self.dot_density = Param(0.3)
        self.dot_radius  = Param(0.6)
        self.t_pre       = Param(30.0)   # 0% coherence
        self.t_closed    = Param(40.0)   # closed-loop block
        self.t_post      = Param(10.0)   # 0% coherence
        self.vigor_thr   = Param(30.0)   # blackout threshold
        self.hyst        = Param(5.0)    # hysteresis to reduce flicker
